...
# Load embedding model globally (replaces Jina)
embedding_model = SentenceTransformer('all-MiniLM-L6-v2')

def extract_text_from_pdf(cv_file):
    """
    Extracts text using pdfplumber.
    Better at handling multi-column layouts than fitz.
    """
    text = ""
    try:
...
def process_application(application_instance):
    ...
    try:
        ...
        # We keep GLiNER from AppConfig
        gliner = JobsConfig.gliner_model
        # We no longer need Jina from AppConfig
        ...
        for skill in HARD_SKILLS_DB:
            ...
            if skill not in existing_skills_lower:
                if re.search(r'\b' + re.escape(skill) + r'\b', text_lower):
                    ...
                    # Add to list used for Embedding
    ...
    # Embedding (Updated to SentenceTransformer)
    ...
    try:
        # Replaced Jina encode with local model encode
        application_instance.cv_embedding = embedding_model.encode(rich_context).tolist()
    except Exception as e:
